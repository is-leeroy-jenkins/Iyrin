/*
 * ==========================================================================================
 *  Iyrin Documentation JavaScript
 *  File: docs/assets/js/iyrin.js
 *
 *  Purpose:
 *      Provides safe progressive enhancements for the Iyrin MkDocs Material site.
...
( function()
{
	...
	const IyrinDocs = {
		config: {
			siteName: "Iyrin",
			scrollTopId: "iyrin-scroll-top",
			pageToolsId: "iyrin-page-tools",
			progressId: "iyrin-reading-progress",
			apiSearchId: "iyrin-api-search",
			tableFilterClass: "iyrin-table-filter",
			headingLinkClass: "iyrin-heading-link",
			codeLabelClass: "iyrin-code-label",
			codeToggleClass: "iyrin-code-toggle",
			initializedAttribute: "data-iyrin-enhanced",
			navScrollKey: "iyrin-docs-nav-scroll",
			...
			tableSelector: ".md-typeset table:not([data-iyrin-no-filter])",
		...
		handleDocumentClick: function( event )
		{
			...
			if( target.closest( "[data-iyrin-copy-heading]" ) )
			{
				event.preventDefault();
				this.copyHeadingLink( target.closest( "[data-iyrin-copy-heading]" ) );
				return;
			}
			if( target.closest( "[data-iyrin-copy-page]" ) )
			{
				event.preventDefault();
				this.copyPageLink( target.closest( "[data-iyrin-copy-page]" ) );
				return;
			}
			if( target.closest( "[data-iyrin-print-page]" ) )
			{
				event.preventDefault();
				window.print();
				return;
			}
			if( target.closest( "[data-iyrin-toggle-code]" ) )
			{
				event.preventDefault();
				this.toggleCodeBlock( target.closest( "[data-iyrin-toggle-code]" ) );
				return;
			}
			if( target.closest( "[data-iyrin-api-expand]" ) )
			{
				event.preventDefault();
				this.setApiDetailsState( true );
				return;
			}
			if( target.closest( "[data-iyrin-api-collapse]" ) )
			{
				event.preventDefault();
				this.setApiDetailsState( false );
				return;
			}
			if( target.closest( "[data-iyrin-api-clear]" ) )
			{
				event.preventDefault();
				this.clearApiFilter();
			}
			...
			links.forEach( function( link )
			{
				try
				{
					...
					if( url.host && url.host !== currentHost )
					{
						...
						link.classList.add( "iyrin-external-link" );
						if( !link.querySelector( ".iyrin-external-indicator" ) )
						{
							const indicator = document.createElement( "span" );
							indicator.className = "iyrin-external-indicator";
		...
		customizeSearch: function()
		{
			...
			{
				input.setAttribute( "placeholder", "Search Iyrin docs..." );
				input.setAttribute( "aria-label", "Search Iyrin documentation" );
			} );
		},
		...
		addScrollTopButton: function()
		{
			...
			button.type = "button";
			button.className = "iyrin-scroll-top";
		...
		addPageTools: function()
		{
			...
			tools.className = "iyrin-page-tools";
			tools.innerHTML = [
				"<button type=\"button\" data-iyrin-copy-page title=\"Copy page link\" aria-label=\"Copy page link\">Copy link</button>",
				"<button type=\"button\" data-iyrin-print-page title=\"Print page\" aria-label=\"Print page\">Print</button>"
			].join( "" );
		...
		{
			const button = document.querySelector( "[data-iyrin-copy-page]" );
			this.copyTextToClipboard( window.location.href, button, "Copied", "Copy link" );
		},
		addHeadingLinks: function()
		{
			...
			{
				if( heading.querySelector( "." + IyrinDocs.config.headingLinkClass ) )
				{
					return;
				}
				const button = document.createElement( "button" );
				button.type = "button";
				button.className = IyrinDocs.config.headingLinkClass;
				button.setAttribute( "data-iyrin-copy-heading", heading.id );
		...
		{
			const id = button.getAttribute( "data-iyrin-copy-heading" );
			if( !id )
			{
				return;
			}
		...
		copyTextToClipboard: function( text, button, successText, defaultText )
		{
			...
			if( navigator.clipboard && typeof navigator.clipboard.writeText === "function" )
			{
				...
				{
					IyrinDocs.fallbackCopyText( text );
					updateButton();
				} );
				return;
			}
		...
		addTableFilters: function()
		{
			...
			{
				if( table.getAttribute( "data-iyrin-filtered" ) === "true" )
				{
					return;
				}
				...
				if( rows.length < IyrinDocs.config.largeTableMinimumRows )
				{
					return;
				}
				table.setAttribute( "data-iyrin-filtered", "true" );
				const wrapper = document.createElement( "div" );
				wrapper.className = "iyrin-table-tools";
				const input = document.createElement( "input" );
				input.type = "search";
				input.className = IyrinDocs.config.tableFilterClass;
				input.placeholder = "Filter table...";
				input.setAttribute( "aria-label", "Filter table " + ( index + 1 ) );
				const count = document.createElement( "span" );
				count.className = "iyrin-table-count";
				...
				{
					IyrinDocs.filterTable( table, input.value, count );
				} );
			} );
		},
		...
		addCodeLabels: function()
		{
			...
			codeBlocks.forEach( function( code )
			{
				...
				if( !pre || pre.getAttribute( "data-iyrin-labeled" ) === "true" )
				{
					return;
				}
				const language = IyrinDocs.detectCodeLanguage( code );
				if( !language )
				{
					return;
				}
				pre.setAttribute( "data-iyrin-labeled", "true" );
				const label = document.createElement( "div" );
				label.className = IyrinDocs.config.codeLabelClass;
		...
		addCodeToggles: function()
		{
			...
			{
				const pre = code.parentElement;
				if( !pre || pre.getAttribute( "data-iyrin-toggle-ready" ) === "true" )
				{
					return;
				}
				pre.setAttribute( "data-iyrin-toggle-ready", "true" );
				if( pre.scrollHeight <= IyrinDocs.config.maxCollapsedCodeHeight + 80 )
				{
					return;
				}
				pre.classList.add( "iyrin-code-collapsed" );
				pre.style.maxHeight = IyrinDocs.config.maxCollapsedCodeHeight + "px";
				const button = document.createElement( "button" );
				button.type = "button";
				button.className = IyrinDocs.config.codeToggleClass;
				button.setAttribute( "data-iyrin-toggle-code", "collapsed" );
		...
		toggleCodeBlock: function( button )
		{
			...
			const state = button.getAttribute( "data-iyrin-toggle-code" );
			if( state === "collapsed" )
			{
				pre.classList.remove( "iyrin-code-collapsed" );
				pre.style.maxHeight = "";
				button.setAttribute( "data-iyrin-toggle-code", "expanded" );
		...
		{
			const content = document.querySelector( this.config.contentSelector );
			if( !content || content.querySelector( ".iyrin-page-path" ) )
			{
				return;
			}
			...
			meta.className = "iyrin-page-path";
		...
		enhanceKeyboardFocus: function()
		{
			document.body.addEventListener( "keydown", function( event )
			{
				if( event.key === "Tab" )
				{
					document.body.classList.add( "iyrin-keyboard-mode" );
				}
			} );
			document.body.addEventListener( "mousedown", function()
			{
				document.body.classList.remove( "iyrin-keyboard-mode" );
			} );
		},
		enhanceApiReference: function()
		{
			...
			{
				if( container.getAttribute( "data-iyrin-api-enhanced" ) === "true" )
				{
					return;
				}
				container.setAttribute( "data-iyrin-api-enhanced", "true" );
				const heading = container.querySelector( "h2, h3, h4, h5" );
				if( !heading || heading.querySelector( ".iyrin-api-badge" ) )
				{
					return;
				}
				const badge = document.createElement( "span" );
				badge.className = "iyrin-api-badge";
		...
		{
			const content = document.querySelector( this.config.contentSelector );
			if( !content || content.querySelector( ".iyrin-api-tools" ) )
			{
				return;
			}
			...
			panel.className = "iyrin-api-tools";
			panel.setAttribute( "aria-label", "API tools" );
			panel.innerHTML = [
				"<h2 class=\"iyrin-api-tools-title\">API Tools</h2>",
				"<label class=\"iyrin-api-search-label\" for=\"" + this.config.apiSearchId +
				"\">Filter classes, functions, methods, properties, or text</label>",
				"<input id=\"" + this.config.apiSearchId +
				"\" class=\"iyrin-api-search\" type=\"search\" placeholder=\"Filter API reference...\" autocomplete=\"off\">",
				"<div class=\"iyrin-api-tool-buttons\">",
				"<button type=\"button\" class=\"iyrin-api-tool-button\" data-iyrin-api-expand>Expand all</button>",
				"<button type=\"button\" class=\"iyrin-api-tool-button\" data-iyrin-api-collapse>Collapse all</button>",
				"<button type=\"button\" class=\"iyrin-api-tool-button\" data-iyrin-api-clear>Clear filter</button>",
				"</div>",
				"<p class=\"iyrin-api-filter-status\" aria-live=\"polite\"></p>"
			].join( "" );
			...
			if( input )
			{
				input.addEventListener( "input", function()
				{
					IyrinDocs.filterApiObjects( input.value );
				} );
			}
		},
		filterApiObjects: function( query )
		{
			...
			const status = content.querySelector( ".iyrin-api-filter-status" );
			let visible = 0;
			objects.forEach( function( object )
			{
				...
				if( !normalizedQuery || text.indexOf( normalizedQuery ) !== -1 )
				{
					object.classList.remove( "iyrin-api-hidden" );
					visible += 1;
				}
				else
				{
					object.classList.add( "iyrin-api-hidden" );
				}
			} );
			if( status )
			{
		...
		enhanceTocProgress: function()
		{
			...
			links.forEach( function( link )
			{
				if( !link.querySelector( ".iyrin-toc-marker" ) )
				{
					const marker = document.createElement( "span" );
					marker.className = "iyrin-toc-marker";
		...
		updateTocProgress: function()
		{
			...
			tocLinks.forEach( function( link )
			{
				...
				{
					link.classList.add( "iyrin-toc-active" );
				}
				else
				{
					link.classList.remove( "iyrin-toc-active" );
				}
			} );
		},
		addMermaidGuard: function()
		{
			...
			{
				if( block.getAttribute( "data-iyrin-mermaid-guard" ) === "true" )
				{
					return;
				}
				block.setAttribute( "data-iyrin-mermaid-guard", "true" );
	...
	{
		document.addEventListener( "DOMContentLoaded", initializeIyrinDocs );
	}
	else
	{
		initializeIyrinDocs();
